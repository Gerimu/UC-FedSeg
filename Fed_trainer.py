...
def trainer_synapse(args, model, snapshot_path, root_path, list_dir, num_classes):
    ...
    db_train = Synapse_dataset(base_dir=root_path, list_dir=list_dir, split="train_part",
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs, afma_pred, att = model(image_batch)  # begin
...
def trainer_amos(args, model, snapshot_path, root_path, list_dir, num_classes):
    from datasets.dataset_synapse import Synapse_dataset, RandomGenerator
    # from datasets.dataset_acdc import Synapse_dataset, RandomGenerator
    logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
...
def trainer_flare(args, model, snapshot_path, root_path, list_dir, num_classes):
    from datasets.dataset_synapse import Synapse_dataset, RandomGenerator
    # from datasets.dataset_acdc import Synapse_dataset, RandomGenerator
    logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
