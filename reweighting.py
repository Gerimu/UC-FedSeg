...
def weight_learner(cfeatures, x5, args, global_epoch=0, iter=0):
    ...
    # avgpool = nn.AdaptiveAvgPool2d(56)   # gao
    linear = nn.Linear(2048, 768).cuda()
    x5 = linear(x5)
    all_feature = torch.cat([cfeaturec, x5.detach()], dim=0)
    # all_feature = cfeaturec
    optimizerbl = torch.optim.SGD([weight], lr=args.lrbl, momentum=0.9)
    # optimizerbl = torch.optim.Adam([weight], lr=0.001, weight_decay=0.0001)

    for epoch in range(args.epochb):
        lr_setter(optimizerbl, epoch, args, bl=True)
        all_weight = torch.cat((weight, weight), dim=0)
        # all_weight = weight
