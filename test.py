...
# from datasets.dataset_acdc import Synapse_dataset           #ACDC gao
...
Data_List = ['Synapse', 'AMOS', 'FLARE22', 'WORD']
Dataset_name = Data_List[2]
parser = argparse.ArgumentParser()
parser.add_argument('--volume_path', type=str,
                    default='F:/'+Dataset_name, help='root dir for validation volume data')  # for acdc volume_path=root_dir
# parser.add_argument('--volume_path', type=str,
...
parser.add_argument('--output_dir', type=str,
                    default='./output/FLARE22/AFMA_Conv_UN_14class_0.2.4', help='output dir')
...
parser.add_argument('--test_save_dir', type=str, default='./predictions/test', help='saving prediction as nii!')
...
def inference(args, model, test_save_path=None):
    ...
    db_test = args.Dataset(base_dir=args.volume_path, split="test_part", list_dir=args.list_dir)     #ACDC gao
    ...
    for i in range(1, args.num_classes):    # word 数据集没有aorta
...
if __name__ == "__main__":
    ...
    dataset_config = {
        ...
        'WORD': {
            'Dataset': Synapse_dataset,
            'volume_path': args.volume_path,  # ../data/Synapse/train_npz
            'list_dir': './lists/lists_WORD',  # ./lists/lists_Synapse
            'num_classes': 9,
            'z_spacing': 3,
        },
    }
    ...
    log_folder = './test_log/Ablation/'
    if not os.path.exists(log_folder + args.output_dir.split('/')[-1]):
        os.makedirs(log_folder + args.output_dir.split('/')[-1])
    ...
    if args.is_savenii:
        args.test_save_dir = os.path.join('D:\Project_fan\Swin-AFMA\predictions/', args.output_dir)
        test_save_path = args.test_save_dir + Dataset_name
